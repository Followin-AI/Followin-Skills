...
└── c6_ticker-check.md        # 标的速查（群内问答支持，随时触发，最高频）
.claude/references/
...
定位：群内问答支持。群成员问"XX 怎麼看"，运营把 ticker 丢给 skill，30 秒拿到**回复体**（非贴文体：更口语、无标题排版、适合直接贴在对话串里回复）。

调用序列（≈2 点/次）：
| 步骤 | 调用 | 额度 |
|---|---|---|
| 1 | `metrics(query="<TICKER> price target analyst ratings", asset_type="tradfi")` 快照 + 分析师共识（中位/最高/最低目标价） | 1 |
| 2 | `news(query="<TICKER> <公司名>", time_range="7d", limit=5)` 近期新闻一条最重要的 | 0（实测） |
| 3 | （可选，热议标的才加）`signal(query="<TICKER> 详细仓位", …)` 补内部人/喊单一句 | 1 |

回复体固定五件套（250-350 字）：**現價與位置**（对 52 周区间）→ **機構怎麼看**（共识目标价+分歧幅度）→ **最近发生了什么**（一条）→ **要小心什么**（一句风险）→ 一句免责（短版："不是投資建議，功課要自己做"）。

防坑规则：
- 群成员什么票都会问：非美股正股（代币化/OTC/仙股/加密）直接回"這檔不在覆蓋範圍"模板，不硬答。
- 共识目标价必须带家数和分歧（"12 家机构，中位 $X，最高最低差 Y%"）——单均值误导性大（lynette 页教训）。
- 同一标的当日已查过（对话内可见）→ 直接复用，不重复烧额度。
- 问的若是"该不该买"：回复体模板固定引导到"三個你自己要回答的問題"（仓位/周期/跌 20% 怎么办），绝不接盘决策——风格 SSOT 禁令的对话版执行。
