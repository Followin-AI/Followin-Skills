...
来源：可见 N 篇研报（上游硬顶 10 篇）｜共 M 条催化剂
...
· 时间精度：日 N 条 / 月 N 条 / 半年·年 N 条 / 待锚定 N 条
```

**⛔ 硬要求**：
- **不许把粗粒度渲染成精确日期。** `2028` 就写 2028，不写 2028-01-01——哪怕 `sort` 字段是那么给的。
- **不许丢弃待锚定与已过期两桶。** 前者是有信息无时间，后者能回看兑现率——都比静默删掉有用。
- **点名家数要标。** 1 家点名和 4 家独立点名是两回事。
- **解读给判断不复述表。** 例："未来 90 天只有一个日级节点（AMD 发布会），其余全在半年级以上——**这标的近期没有研报意义上的时间催化**"。
...
| 只能看 10 篇，且**不一定给满** | 上游硬顶（N-38）| 覆盖标下界。实测 F 只返回 3 篇 |
