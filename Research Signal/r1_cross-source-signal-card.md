...
**⛔ 硬要求**：
- **不许只吐表**——必须点明什么值得停下看、什么可忽略、异常主动顶出。解读给判断不复述数字（例："三家全维持、评级动作零变化，真正的读数是**没人变心**，不是'机构看好'"）。
- **不给买卖建议、不给仓位。** 输出是"值得进一步研究的校准读法"。
- **跨源打架时明说，不硬圆。** 四维里任一维是"无数据"或"不可判"，照写。
- **价格只引本次 MCP 返回值**（红线：新闻与 KOL 转述的百分比是二手，必经快照核实）。
