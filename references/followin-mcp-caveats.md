...
| N-34 | **`earnings call transcript` 可能返回滞后一季的逐字稿，且事前无信号可预判**：实测 TER `beat_miss.date=2026-07-28`(Q2) 但 transcript 返回 `date=2026-04-29 / period=Q1`，正文抬头「Teradyne **First Quarter** 2026 Earnings Conference Call」。该股三道差集全过、四道硬闸全过、财报日期新鲜，**无任何字段提示逐字稿会滞后**，1 次额度买到废件。<br>⚠️ **同时修正 N-28 的理由**：`_meta.freshness` 恒为 `"q-1"` —— 对 INTC 是**谎话**（内容实为本季 Q2），对 TER 却是**实话**（确为上一季 Q1）。**恒定输出即不携带信息**，对错各半，规避动作不变但理由从"该字段总是错"改为"该字段恒定，故无论对错都不可用" | 拉取后必须核对 `transcript[0].date`/`period`/`year` 与 `beat_miss.date` 是否同季。**不匹配时该股结论标注"欠测"而非低分**——低分=扫了没讲，欠测=根本没扫到，两者对下一步动作的指示相反 | 实测 2026-07-29 |
