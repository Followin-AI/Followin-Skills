# 14 Multi-Agent Stock Analysis — 21 Agent Prompts（自 v1 提取，业务逻辑稳定层）

> 本文件是 14_multi-agent-stock-analysis_v2 的引用附件：19 位分析师 + 风控经理 + 组合经理的完整 prompt（投资哲学 / 关注数据子集 / 评分框架 / 信号阈值）。
> 来源：v1 原文件 L145-789（git ed2fad4^），内容未改动。
