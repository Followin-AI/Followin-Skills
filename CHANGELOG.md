...
- `Feed Manager` = **你自己那条信息流**（拆解 + 归档 + 长期管理）

**目录形态取 Twitter Workflow 的先例**：从外部开发主源同步的快照，`skills/` 下放实际 skill 目录，README 顶部挂主源指针，安装用 `cp -rn "Feed Manager/skills/"* ~/.claude/skills/`。不是单文件 command——它带 6 个 references + 2 个 scripts，塞不进 `~/.claude/commands/`。
...
**定名过程**（同日改过两轮：`KOL Watch/` → `Feed Desk/` → `Feed Manager/`）：`KOL` 在本仓不唯一——`c4_social-pulse` 也是 KOL，当目录名区分不出是哪一种；`Watch` 又与 Premarket **Tracker** 的动作重叠。域词最终取 **Feed（信息流）**——兄弟目录的域词全是通用词（Earnings / Premarket / Research / Trader / Twitter），roster、watchlist 这类行话或歧义词不合格（watchlist 在金融语境指标的而非人）。动作词取 **Manager**：这支的职能就是**把一条持续流入的信息流管起来**——每天拆解、逐日归档、长期可查，而不是某一次性动作。排除过 `Roster Journal` / `Roster Digest` / `Watchlist Journal` / `Following Digest` / `Feed Reader`（会被读成 RSS）/ `Feed Archive`（像冷存储，弱化了每天主动跑）/ `Feed Desk`。

**验证状态如实标注**：主链已端到端实跑（🅱️ 简报 + 🅰️ 全流程含门禁反向测试）；reader / completeness-critic 子代理与 5 节周报**仅模板验证、尚未实跑**——README 与根 README 均已写明，不粉饰。

## 2026-08-05 — N-29 判据放宽：从「反号」到「不相等」（源自 Evose 移植实测）

**起因是一次意外收获。** 把 Research Reader 移植到 Evose 平台做验收时，Agent 产出了两个假陈述：
「上季 EPS 2.40 → 下季预期 2.08 = 利润下滑 13%」和「实际 2.40 vs 预期 1.76 超预期 6.3%」（该式实为 +36%，自身不成立）。

**回查源 Skill 才发现 r1 其实有 N-29 的闸，但触发条件太窄**——原文写「`beat_miss.epsActual` 与 `latest_quarter.eps` **反号**即判定口径错位」，那是照 INTC 定的（`+0.42` vs `−2.16`，一正一负很扎眼）。

**而 NVDA 是同号不同值**：实测同一季度 `earnings_surprise.actual_eps = 1.87` vs `financial_statement.eps = 2.40`，都为正、差 **28%**，**按反号判据完全不触发**（request_id `ec350d7b…`）。

🔴 **同号错位比反号更危险**：反号一眼看得出不对劲，同号会被当成同一个数直接混用。用同一条序列算，`1.87 → 2.08` 是 **+11%**，与营收 +12.6% 完全吻合——那个"利润下滑"是口径混用造出来的，不是数据本身。

**判据改为「两个 EPS 只要不相等即判定口径错位」**，并分两种处置：
- **反号**（如 INTC）→ 标注「该超预期为非 GAAP 口径」，改用营收 surprise 作主锚
- **同号不等**（如 NVDA）→ 两条序列都可用但**严禁跨序列组合**：与 `next_earnings_estimate.epsEstimated` 比只能用 `actual_eps`；看历史趋势只能用 `eps_trend`（与 `financial_statement` 同源）

**同步范围**：caveats N-29 主条 + workaround 列 · `Research Reader/r1` 闸 1 · `Base Skill/01`、`02`（两处）· `Earnings Screener`（两处）——共 4 文件 8 处，全仓「反号即」表述已清零。

💡 **方法论收获**：闸门的**触发条件**和闸门本身一样重要。这条闸存在了一周，但因为判据是照单一案例（INTC）定的，遇到形态不同的同类问题就整个失效。**以后立闸时要问：这个判据是从案例的本质抽出来的，还是从案例的表象抽出来的？**

## 2026-08-04（晚）— caveats 文件结构优化：93 个编号不变，正文从"一堵墙"变三层

**起因是一句用户吐槽：「caveats 怎么越改越多了」。** 算账后确认：93 条里真正等 dev 的硬 bug 只有 11 条上下，绝大多数是「读法纪律」（数据的脾气，永远不会"修好"）——但单文件 93 行大表四种记录混排，打开看不出"哪些是债、哪些是资产"。

**三个动作（全部不重编号——全仓上百处 `见 N-xx` 引用，ID 只增不改）：**

1. **文件头加「📊 状态总览」**：93 = 正文 80 + 存根 4 + 归档 9；等 dev 的硬问题按家族速览（权威仍在正文）；维护纪律三条写死。
2. **根因家族归并（4 条存根化）**：
   - **数组参数家族 → N-8 主条**：吸收 N-71（服务端建议自相矛盾 + schema 根因 + 一行修法）；N-59f 裁掉重复前半、保留独立的 fanout 发现；N-31（107 处示例盘点）随已修入归档
   - **query 解析家族 → N-23（主条①：截断/字典缺失/解析成功≠有数据/指数重复）+ N-69（主条②：英文词劫持）**：N-12、N-14、N-18 存根化并入；N-36（影子代码挤名额）保留为独立机制成员
3. **9 条已修/已作废移入文末归档段**：N-2 · N-11 · N-17 · N-31 · N-61 · N-37/N-37a/N-37b/N-37·状态。销案不删条，ID 永久可检索，回归自查按各条验收证据复测。

**校验**：ID 全集逐一 diff 一致（93→93，零丢失零重复）· 全部条目列数正常 · 3 个主条均为实体非存根 · 203 个链接 0 失效。

**没做的（刻意）**：不按主题重排批次结构——文件本质是实验室笔记，按实测批次排列承载着 request_id 级证据链；跨批次物理合并会毁掉审计线。「纪律下沉给产品」那层（~25 条可由服务端一个字段退休）单独成清单，等 dev 有空再提。
