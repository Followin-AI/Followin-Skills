...
| **[`Feed Manager/`](./Feed%20Manager/)** | 1 个 Skill | 美股信息流管理——把你自己关注的那十几个账号，每天**拆开**（按标的/板块/反方拆成结构化条目）并**归档**（逐日累积成可查档案 + 决策日志）。三道门禁堵住静默漏账号/漏推文/漏落盘；每个数字必须可追溯（含[目录 README](./Feed%20Manager/)）|
...
三道门禁自动跑，各堵一类静默失效：**账号级**覆盖表（该拉的都拉了吗）、**推文级**翻页告警（一次调用只返一页 20 条，宽窗口盖不住会静默漏推）、**文件级**收尾 hook（拉到的都写了吗）。外加一条全程硬约束：**每个数字必须能追溯到数据源或明确原则——没依据就不给数字。**
