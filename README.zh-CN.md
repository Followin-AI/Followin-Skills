...
npx @followin/skills setup --client opencode-project # OpenCode / OpenClaw(skill 项目级,MCP 全局)
npx @followin/skills setup --client opencode        # OpenCode / OpenClaw(全局)
```
...
| 客户端 | 配置文件路径 |
|---|---|
| Claude Code | `~/.claude.json`(全局)或 `<cwd>/.mcp.json`(项目级) |
