...
**券商研报说的话，到底能不能信、该信到什么程度、接下来该盯哪天、还有谁被卷进来。** 完整说明见 **[`Research Desk/README.md`](./Research%20Desk/README.md)**。

| # | 模块 | 干什么 |
|---|---|---|
| **r1** | 跨源印证读数卡 | 一份研报说"目标价 350"该不该信——拿全街共识、股价走势、网红与高管动向、最近一季财报四边对撞。**四边对得上才算数** |
...
每只股票**最多只返回 10 篇**去重后常只剩三到五家机构。所以四支全是深度工具，一支扫描器都没有——
**是数据不支持，不是做个打折版糊弄过去**。理由与实测证据见目录 README。

⚠️ 有个上限躲不掉：每只股票最多 10 篇、去重后常 3–5 家。**任何"N 家机构怎么看"都是下限不是全街**，四支都被要求把这句写进输出。

✅ **四支都经实测撞过真数据**：r1–r3 做过端到端实跑（NVDA 勘探 + INTC/GOOGL/F 换标的，规格写成脚本），**抓出并修掉 6 个规格缺陷**；r4 的设计被实测**推翻过两次**（枢纽票反而没产出、同框名字彼此无关）。清单见目录 README。
